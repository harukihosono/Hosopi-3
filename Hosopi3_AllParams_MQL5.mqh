...
sinput group "== 個別利確幅指定 =="
input string IndividualTPTitle = "ポジション別利確幅（上記可変利確ONで有効）"; // 個別利確設定
